...
// 		imagestring($this->im,2,($pos["x1"]+5),($pos["y1"]-$this->heights["task"]-3),$d,$this->colors["font"]);
		...
		$Image_Canvas_Tool=new Image_Canvas_Tool();
		imagettftext($this->im,10,0,($pos["x1"]+5),($pos["y1"]-$this->heights["task"]+9),$this->colors["font"], $Image_Canvas_Tool->fontMap("DroidSansFallback"),$d);
...
// 		imagestring($this->im,2,($pos["x3"]+5),($pos["y2"]-$this->heights["task"]-3),$this->data_gantt[$id]["label"],$this->colors["font"]);
		require_once('Image/Canvas/Tool.php');
		$Image_Canvas_Tool=new Image_Canvas_Tool();
		imagettftext($this->im,10,0,($pos["x3"]+5),($pos["y2"]-$this->heights["task"]-3),$this->colors["font"],$Image_Canvas_Tool->fontMap("DroidSansFallback"),$this->data_gantt[$id]["label"]);
	}
